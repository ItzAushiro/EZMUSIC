...
module.exports = {
  name: "play",
  aliases: ["p"],
  description: "Joue la chanson de YouTube/Stream",
  cooldown: 1.5,
...
async execute(message, args, client) {
    ...
    //If no args return
    if (!args.length)
      return attentionembed(message, `Usage: ${message.client.prefix}play <YouTube URL | Nom de la vidéo | Soundcloud URL>`);
      ...
      if (serverQueue) {
        ...
        if (urlValid) { //send searching link
          message.channel.send(new MessageEmbed().setColor("RED")
            .setDescription(`**:notes: En train de chercher... 🔍 [\`LINK\`](${args.join(" ")})**`))
        //if not
        }
        else { //send searching TITLE
          message.channel.send(new MessageEmbed().setColor("RED")
            .setDescription(`**:notes: En train de chercher... 🔍 \`${args.join(" ")}\`**`))
        }
      } else {
